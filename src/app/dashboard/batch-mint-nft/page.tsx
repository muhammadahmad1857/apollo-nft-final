...
const totalPriceHuman = !isPriceLoading
  ? (Number(mintPriceHuman) * selectedFiles.length).toFixed(4)
  : "0";
    ...
    <div className="max-w-3xl mx-auto mt-10 px-4 w-full">
      <div className="mb-10 text-center">
        ...
        <p className="mt-4 text-lg text-gray-500 dark:text-gray-300 max-w-2xl mx-auto">
          Select multiple files to mint NFTs in a single transaction. Set
          royalty and review quantity before minting. Estimated gas is shown
          below.
        </p>
      </div>
    ...
    saveRoyalty(val, "BATCH"); // save in sessionStorage whenever user updates
        ...
        <div className="flex flex-col gap-4 mt-4">
          ...
          <div className="rounded-lg bg-cyan-50 dark:bg-cyan-900/40 p-3 text-cyan-800 dark:text-cyan-200 text-xs font-semibold flex items-center gap-2 animate-fade-in">
            ...
            Did you know? Batch minting saves you time and gas compared to
            minting one by one!
          </div>
          {/* Batch Mint Price Info */}
<div className="rounded-xl border border-cyan-200/60 dark:border-cyan-800/60 bg-cyan-50/60 dark:bg-cyan-950/40 p-4 shadow-sm transition-all">
  <div className="flex items-start gap-3">
    <SparklesIcon className="w-5 h-5 text-cyan-600 dark:text-cyan-300 mt-0.5" />
    ...
    <div className="flex-1">
      ...
      {isPriceLoading ? (
        <div className="mt-2 space-y-2">
          <div className="h-4 w-28 rounded bg-cyan-200/60 dark:bg-cyan-800/60 animate-pulse" />
          <div className="h-3 w-40 rounded bg-cyan-100/60 dark:bg-cyan-900/60 animate-pulse" />
        </div>
      ) : (
        <>
      ...
      <p className="mt-2 text-xs text-gray-400 dark:text-gray-500">
        Price is fetched directly from the smart contract and calculated automatically.
      </p>
    </div>
  </div>
</div>
        ...
        <Button
          ...
          className="my-4 py-3  text-base font-semibold rounded-xl shadow-md bg-cyan-600 hover:bg-cyan-700 text-white dark:bg-cyan-500 dark:hover:bg-cyan-400 transition-all duration-200"
        >
          {isBusy || isMinting ? (
