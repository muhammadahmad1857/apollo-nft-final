...
//     // First: log everything we see on this target so we know what's there
//     try {
...
//         ownKeys,
//         protoKeys,
//         // Log values of any object-type own keys (these could be the chains map)
//         objectProps: ownKeys.reduce((acc, k) => {
//           try {
//             const v = target[k];
//             if (v && typeof v === "object" && !Array.isArray(v) && !(v instanceof Function)) {
//               acc[k] = Object.keys(v).slice(0, 10); // first 10 keys of any object prop
//             }
//           } catch { /* ignore */ }
//           return acc;
...
//             listener(accounts);
//           }
//         : (...args: any[]) => { logApollo("info", `Event: ${event}`, args); listener(...args); };
...
//       logApollo("info", "Pre-patching chains map on EIP-6963 discovery...");
...
function isProvider(obj: any) {
  return obj && typeof obj.request === "function";
}

function getProvider(): any {
  const sdk = getApolloWalletSdk();
  if (isProvider(sdk)) return sdk;

  const win = window as any;
  return win.ethereum;
}

function timeoutPromise<T>(p: Promise<T>, ms: number, label: string) {
  return Promise.race([
    p,
...
async function addApolloChain(provider: any) {
  try {
    ...
    log("info", "Chain added successfully");
  } catch (err) {
    log("warn", "wallet_addEthereumChain failed (might already exist)", err);
  }
}

async function switchApolloChain(provider: any) {
  try {
    await provider.request({
      method: "wallet_switchEthereumChain",
      params: [{ chainId: APOLLO_CHAIN_HEX }],
    ...
    log("info", "Switched to Apollo chain");
  } catch (err: any) {
    log("warn", "wallet_switchEthereumChain failed", err?.message);
  }
}
...
export async function connectApolloWallet(): Promise<string[]> {
  ...
  if (!provider) {
    throw new Error("Apollo Wallet / Ethereum provider not found");
  }

  try {
    log("info", "Step 1: Add chain");
    await addApolloChain(provider);

    log("info", "Step 2: Switch chain");
    await switchApolloChain(provider);

    log("info", "Step 3: Request accounts");
    const accounts = await requestAccounts(provider);

    log("info", "CONNECT SUCCESS", accounts);
    ...
    throw new Error(
      err?.message ||
        "Apollo Wallet connection failed. Make sure extension is unlocked and Apollo Mainnet is added."
    );
  }
}

/**
 * OPTIONAL: safe provider wrapper (lightweight only)
 */
export function wrapProvider(raw: any) {
  if (!isProvider(raw)) return raw;

  return {
    ...raw,
    request: async (args: any) => {
      log("info", `RPC → ${args.method}`);
      const res = await raw.request(args);
      log("info", `RPC ← ${args.method}`);
      return res;
    },
  };
...
export function isApolloWalletInstalled() {
  ...
  return Boolean(
    sdk ||
      win.ethereum?.isApolloWallet ||
      win.ethereum?.isApollo ||
      win.apolloWallet
  );
}
 ...
 */
export function getApolloWalletProvider() {
  return wrapProvider(getProvider());
}

/**
 * WAIT FOR WALLET (for SSR hydration / lazy inject)
 */
export async function waitForApolloWalletProvider(timeout = 6000) {
  const start = Date.now();

  while (Date.now() - start < timeout) {
    const p = getApolloWalletProvider();
    if (p) return p;
